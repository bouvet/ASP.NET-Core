...
1. Use the instructions in Getting Started to setup an Empty Web Application. Remember to rename all references to `Microsoft.AspNet.*` to `Microsoft.AspNetCore.*`

1. Add `Microsoft.AspNetCore.Mvc.Core` to `project.json`:
...
1. Add an attribute route `[Route("/api/products")]` to the `ProductsController` class (you may have to add `using Microsoft.AspNetCore.Mvc`):
...
1. Add a `Get` method to `ProductsController` that returns a `string` "Hello API World" with an attribute route
  ...
  public class ProductsController
  {
    public string Get() => "Hello API World";
  }
  ```

1. Run the application and navigate to `/api/products`, it should return the string "Hello API World".
